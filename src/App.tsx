...
export default function App() {
  ...
  // 25 seconds to fill → 100/25 = 4% per second → at ~60fps that's ~0.067 per frame
  const handleInteraction = () => {
    if (step === 'virus' && spinProgress < 100) {
      lastInteractionRef.current = Date.now();
      setSpinProgress(prev => Math.min(prev + 0.15, 100));
    }
  };

  // Fallback: lose 5% every second when user stops dragging
  useEffect(() => {
    if (step !== 'virus') return;

    const interval = setInterval(() => {
      const elapsed = Date.now() - lastInteractionRef.current;
      // If no interaction for over 400ms, start decaying
      if (elapsed > 400) {
        setSpinProgress(prev => Math.max(prev - 5, 0));
      }
    }, 1000);
